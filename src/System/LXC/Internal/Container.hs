...
mkCreateFn = error "not implemented"
...
mkGetRunningConfigItemFn = error "mkGetRunningConfigItemFn"
...
mkGetInterfacesFn = error "mkGetInterfacesFn"
...
mkSnapshotListFn = error "mkSnapshotListFn"
...
mkConsoleGetFDFn = error "mkConsoleGetFDFn"
...
setItemFn g k v = error "setItemFn"
...
getDaemonize = error "getDaemonize"
...
start :: Bool       -- ^ Use @lxcinit@ rather than @\/sbin\/init@.
      ...
      -> LXC Bool   -- ^ @True@ on success, else @False@.
start useinit argv = error "start"
...
attach exec cmd opts = error "attach"
...
snapshotList = error "snapshotList"
...
create t bdevtype bdevspecs flags argv = error "create"
